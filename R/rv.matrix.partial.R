...
#' @encoding UTF-8
#' @export
rv.matrix.partial <- function (mx1, mx2, x, my1 = NULL, my2 = NULL, y, mz1 = NULL, mz2 = NULL, z,
                                ...
                                seqpermutation = NULL, parallel = NULL, newClusters = TRUE, CL =  NULL)
{
  ...
  z <- as.matrix(z)
  statistic <- coinertia.partial.syncsa(x, y, z)
  N <- nrow(mx2)
  if(is.null(seqpermutation)){
  ...
  ptest <- function(samp, mx1, mx2, my1, my2, mz1, mz2, permute.my2, permute.mz2, norm, norm.y, norm.z){
    ...
    if(!permute.my2 & !permute.mz2){
      res <- SYNCSA::coinertia.partial.syncsa(x.permut, y, z)
    }
    if(permute.my2 & !permute.mz2){
      res <- SYNCSA::coinertia.partial.syncsa(x.permut, y.permut, z)
    }
    if(!permute.my2 & permute.mz2){
      res <- SYNCSA::coinertia.partial.syncsa(x.permut, y, z.permut)
    }
    if(permute.my2 & permute.mz2){
      res <- SYNCSA::coinertia.partial.syncsa(x.permut, y.permut, z.permut)
    }
    return(res)
  }
  if(is.null(parallel)){
